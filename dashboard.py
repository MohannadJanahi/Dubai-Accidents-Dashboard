...
def resolve_data_source(args) -> str:
    ...
    if not DEFAULT_LOCAL_DATA.exists():
        raise FileNotFoundError(
            f"Local dataset not found at {DEFAULT_LOCAL_DATA}. "
            "Use --online to fetch from Dubai Pulse, or pass --data with a CSV path."
        )
    return str(DEFAULT_LOCAL_DATA)
...
def parse_args():
    ...
    parser.add_argument(
        "--online",
        action="store_true",
        help="Fetch traffic incident data from Dubai Pulse instead of the local CSV.",
    )
    parser.add_argument(
        "--data",
        help="Path to a custom CSV file (overrides the default local file and --online).",
    )
